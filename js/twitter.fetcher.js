...
(function(root, factory) { if (typeof define === 'function' && define.amd) { define([], factory); } else if (typeof exports === 'object') { module.exports = factory(); } else { factory(); } }(this, function() {
    ...
    function handleTweets(tweets) {
        if (customCallbackFunction === null) {
            ...
            var html = '<ul class="list-unstyled recent-tweets">';
            while (n < x) {
                html += '<li>' + tweets[n] + '</li>';
                n++;
            }
    ...
    var twitterFetcher = {
        fetch: function(config) {
            ...
            if (inProgress) { queue.push(config); } else {
                ...
                targetBlank = config.linksInNewWindow || true;
            ...
            if (dataOnly) {
                ...
                while (n < x) {
                    ...
                    if (parseLinks) {
                        ...
                        if (printTime) {
                            if (permalinks) { op += '<p class="timePosted"><i class=\"fa fa-twitter\"></i>&nbsp;&nbsp;<a target="_blank" href="' + permalinksURL[n] + '">' + times[n].getAttribute('aria-label') + '</a></p>'; } else {
                    ...
                    if (showInteractionLinks) {
                        op += '<p class="interact"><a target="_blank" href="https://twitter.com/intent/' + 'tweet?in_reply_to=' + tids[n] + '" class="twitter_reply_icon"' +
                            (targetBlank ? ' target="_blank">' : '>') + 'Reply</a><a target="_blank" href="https://twitter.com/intent/retweet?' + 'tweet_id=' + tids[n] + '" class="twitter_retweet_icon"' +
                            (targetBlank ? ' target="_blank">' : '>') + 'Retweet</a>' + '<a target="_blank" href="https://twitter.com/intent/favorite?tweet_id=' +
